...
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт 
    ...
    Запрос.Текст =
        "ВЫБРАТЬ
        |	ВКМ_ВыполненныеКлиентуРаботыОбороты.СуммаКОплатеПриход КАК СуммаКОплатеРабот,
   ...
   Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НоваяСтрокаТЧУслуги.Номенклатура = Справочники.Номенклатура.НайтиПоНаименованию("Абонентская плата");
        НоваяСтрокаТЧУслуги.Количество = 1;
        НоваяСтрокаТЧУслуги.Цена = ВыборкаДетальныеЗаписи.СуммаАбонентскойПлаты;
        НоваяСтрокаТЧУслуги.Сумма = ВыборкаДетальныеЗаписи.СуммаАбонентскойПлаты; 
		
		Если ВыборкаДетальныеЗаписи.СуммаКОплатеРабот > 0 Тогда
			...
			НоваяСтрока.Сумма = ВыборкаДетальныеЗаписи.СуммаКОплатеРабот; 
...
//    // Строка 1: Абонентская плата
//    НоваяСтрока = Объект.Товары.Добавить();
//    НоваяСтрока.Номенклатура = Справочники.Номенклатура.НайтиПоНаименованию("Абонентская плата");
//    НоваяСтрока.Количество = 1;
//    НоваяСтрока.Цена = СуммаАбонентскойПлаты;
//    НоваяСтрока.Сумма = СуммаАбонентскойПлаты;

//    // Строка 2: Выполненные работы
//    Если СуммаВыполненныхРабот > 0 Тогда
//        НоваяСтрока = Объект.Товары.Добавить();
//        НоваяСтрока.Номенклатура = Справочники.Номенклатура.НайтиПоНаименованию("Выполненные работы");
//        НоваяСтрока.Количество = 1;
//        НоваяСтрока.Цена = СуммаВыполненныхРабот;
//        НоваяСтрока.Сумма = СуммаВыполненныхРабот;
//    КонецЕсли;
//КонецПроцедуры
