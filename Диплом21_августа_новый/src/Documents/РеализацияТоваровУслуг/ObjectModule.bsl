...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	//++Хоменко В.И. 14.09.2026
	//Исходный код. Перенесен в обработчик ОбработкаПроведения  
...
Процедура ОбработкаПроведения(Отказ, Режим)  
	...
	//++Хоменко В.И. 14.09.2026
	Ответственный = ПараметрыСеанса.ВКМ_ТекущийСотрудник;
	//--Хоменко В.И. 14.09.2026
...
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт 
	...
	// 1. Абонентская плата — всегда, из договора
	...
	Если ЗначениеЗаполнено(СуммаАбонентскойПлаты) И СуммаАбонентскойПлаты > 0 Тогда
		...
		НоваяСтрокаТЧУслуги.Цена = СуммаАбонентскойПлаты;
		НоваяСтрокаТЧУслуги.Сумма = СуммаАбонентскойПлаты; 
	КонецЕсли;
	
	// 2. Выполненные работы — только если есть данные в регистре
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
